...
async def on_question(action):
    ...
    try:
        ...
        await cl.Message(content="I didn't quite understand your question. Can you reformulate it ?").send()
        #Peut-être rajouter des suggestions de questions similaires qu'il sait process ???
...
async def suggest_initial_questions(user_profile):
    ...
    response = await cl.make_async(agent.run)(
        f"given what you know about the database, and the following user profile : \n{user_profile}\n\nPlease provide 3 proposals for analytical questions about the data that this type of user might potentially be interested in. Make sure to give three questions which are based on available data present in the graph and on data the user profile has access. Write each question on one single line, do not add anything to the text, it should only contain your questions.")
...
async def settings_updated(settings):
    ...
    if role is not None and previous_role != role:
        ...
        if settings["user_profile"]:
            ...
            #cl.user_session.set('settings', settings)
            await cl.Message(
                content=f'Welcome to the {role}. \n \n Here is the information I have about you: {user_profile} \n \n How can I help you ?').send()
    ...
    tools = [
        ...
        Tool(
            ...
            description="""
                 ...
                 This specialized tool offers streamlined search capabilities to help you find the information you need with ease.
                 If the cypher query contains a WHERE constraint search a string, use always the CONTAINS operator.
                 """
        ),
        # Tool(
...
async def main(message: cl.Message):
    ...
    if suggestions_are_enabled():
        question="based on the previous questions, please generate a few additional questions related to the same topic if relevant. Make sure to propose questions which are based on available data in the graph and on data the user profile has access. If you feel like this is not the time to do so, just output a nice message to tell the user you are there to help."
