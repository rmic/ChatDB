...
Data names used in the query must exactly refer to data names present in the graph.
...
Concerning the database, all data fields concerning time information (charttime, admittime, storetime, admittime, dischtime,...) provides the date (year, month and day) and time of the event. 
The 'gender' data field in 'Patient' nodes provides the gender of the patient which is female when the value is 'F' and male when it's 'M'.
The 'hospital_expire_flag' data field indicates whether the patient died within the hospital by a value 1 or not vy a value 0. 
The 'description' data field in the 'Disease' nodes provides the meaning of the disease ICD code.
The 'LabItem' nodes represents a laboratory concept. The 'LabAnalysis' nodes provides information about the laboratory analysis made on these concepts. 
The 'Procedure' notes represents medical interventions which are described via the 'ProcedureDescription' nodes. These nodes contains the name of the procedures.
